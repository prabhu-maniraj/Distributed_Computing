...
public class InboundAppWorker extends Thread {
	...
	public void run() {
		...
		while (true) {
			System.out.println("Inside while of inbound app worker");
			...
			try {
				...
				if (msg instanceof Request) {
                    ...
                    ConnectionManager.broadcastToAllNodes(req);
				    
					Resource rsc = null;
					if(req.getBody().getClusterMessage().getClusterId()==Server.clusterId)
					{
					rsc=new JobResource();
					System.out.println("Job resource is executed because client is connected to this cluster");
					
					}
					else
					{
						rsc= new ForwardResource();
						System.out.println("Forward Resource is executed to send the request to node in the other cluster");
					}
