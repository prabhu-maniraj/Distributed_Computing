...
logger */
	...
	public static Integer myNodeId;
	public static Integer clusterId=20;
	...
	private void init(File cfg) {
		...
		try {
			...
			System.out.println("node is"+conf.getNodeId());
	...
	private void startManagers() {
		...
		clusterManager=ClusterManager.initManager(conf);
		...
		for (NodeDesc nn : conf.getAdjacent().getAdjacentNodes().values()) {
            ...
            nodeMap.put(nn.getPort(), nn.getHost());
			// fn(from, to)
			HeartbeatPusher.getInstance().connectToThisNode(conf.getNodeId(), node);
		}
		System.out.println("The size of nodemap is"+ nodeMap.size());
		heartbeatMgr.start();
