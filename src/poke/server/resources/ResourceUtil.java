...
//import poke.server.TempServerInitializer;
...
public class ResourceUtil {
	...
	public static Channel getChannel(String nodeIPAddress,Request request){
		...
		try {
			System.out.println("Connecting to "+nodeIPAddress);
			ch = b.connect(nodeIPAddress,5570).sync().channel();
	...
	public static Channel getChannel(String nodeIPAddress, Integer port,Request request){
		EventLoopGroup group = new NioEventLoopGroup();
		...
		try {
			System.out.println("Connecting to "+nodeIPAddress);
			ch = b.connect(nodeIPAddress,port).sync().channel();
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			System.out.println("There is error");
			e.printStackTrace();
